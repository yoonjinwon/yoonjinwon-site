...
(function () {
  ...
  const THEME_BASE =
    (typeof window !== "undefined" && window.WONLAB_THEME && window.WONLAB_THEME.baseUrl) ? String(window.WONLAB_THEME.baseUrl) : "";

  function resolveAssetUrl(url) {
    const u = String(url || "");
    if (!u) return u;
    if (/^(https?:)?\/\//i.test(u) || /^data:/i.test(u)) return u;
    return THEME_BASE + u.replace(/^\//, "");
  }
...
<svg xmlns="http://www.w3.org/2000/svg" width="640" height="400" viewBox="0 0 640 400">
  ...
  function renderPubItem(p) {
    ...
    const thumbCandidateResolved = resolveAssetUrl(thumbCandidate);
    const thumbDefaultResolved = resolveAssetUrl(thumbDefault);
    ...
    const thumbAlt2 = /\.jpe?g$/i.test(thumbCandidate)
      ...
      : /\.png$/i.test(thumbCandidate)
        ...
        : "";
    const thumbAlt1Resolved = thumbAlt1 ? resolveAssetUrl(thumbAlt1) : "";
    const thumbAlt2Resolved = thumbAlt2 ? resolveAssetUrl(thumbAlt2) : "";
    const thumbSrc = escapeHtml(thumbCandidateResolved);
    const thumb = `
      <div class="pub-thumb">
        <img
          src="${thumbSrc}"
          alt=""
          loading="lazy"
          onerror="if(!this.dataset.fallback && ${thumbAlt1Resolved ? "true" : "false"}){this.dataset.fallback='alt1';this.src='${escapeHtml(thumbAlt1Resolved)}';}else if(this.dataset.fallback==='alt1' && ${thumbAlt2Resolved ? "true" : "false"}){this.dataset.fallback='alt2';this.src='${escapeHtml(thumbAlt2Resolved)}';}else if(this.dataset.fallback!=='default'){this.dataset.fallback='default';this.src='${escapeHtml(
            thumbDefaultResolved
